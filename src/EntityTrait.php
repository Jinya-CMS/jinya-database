...
trait EntityTrait
{
    ...
    private function getIdProperty(): array
    {
        /** @var string $filename */
        ...
        $getColumns = static function (string $class) use ($filename): array {
            ...
            foreach ($properties as $property) {
                ...
                if (!empty($cols)) {
                    ...
                    $optionals =$property->getAttributes(Optional::class);
                    if (!empty($optionals)) {
                        /** @var Optional $optional */
                        $optional = $optionals[0]->newInstance();
                        $isRequired = false;
                    ...
                    $result[] = [
                        'column' => new CacheColumn($name, $sqlName, $converter, $column->autogenerated, $isRequired, $defaultValue),
                        'string' => "new " . CacheColumn::class . "('$name', '$sqlName', $converterString, $autogenerated, $required, $defaultValueExported)",
                    ];
...
PHP;
    ...
    private function checkRequiredColumns(): void
    {
        ...
        $missingColumns = [];
        foreach ($columns['byProperty'] as $property => $column) {
            if (!$column->autogenerated && $column->required && !isset($this->{$property})) {
